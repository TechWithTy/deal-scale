...
export async function GET(req: Request) {
  try {
    ...
    if (!propName) {
      console.error("[click-api] No suitable number property for Clicks on page", pageId, Object.keys(resolved.dump?.properties ?? {}));
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ ok: false, error: "notion_patch_failed", status: res.status, details: err }, { status: 500 });
    }
...
async function fetchPageAndResolveClicksProperty(pageId: string, token: string): Promise<{ name: string | null; dump?: NotionPageDump }> {
