...
function pickClicksProperty(props: Record<string, { type?: string; number?: number }>): string | null {
  const candidates = [
    "Redirects (Clicks)",
    "Redirects(Clicks)",
    "Redirects - Clicks",
...
async function findPageIdBySlug(dbId: string, slug: string, token: string): Promise<string | null> {
  ...
  const add = (s?: string) => { if (s && !uniq.has(s)) uniq.add(s); };
  add(slug);
  add(`/${slug}`);
  add(slug.replace(/^\//, ""));
  add(`/${slug}`.replace(/^\//, ""));
...
async function resolveClicksProperty(pageId: string, token: string): Promise<{ name: string | null; keys: string[]; dump?: NotionPageDump; status?: number; statusText?: string }> {
  ...
  if (!resp.ok) return { name: null, keys: [], status: resp.status, statusText: resp.statusText };
...
async function handleIncrement(pageIdOrSlug: { pageId?: string; slug?: string }) {
  ...
  if (!pageId && pageIdOrSlug.slug) {
    pageId = await findPageIdBySlug(DB_ID, pageIdOrSlug.slug.replace(/^\//, ""), NOTION_KEY) ?? undefined;
  }
  ...
  if (!propName) return { ok: false, status: 500, error: "clicks_property_missing", keys: resolved.keys };
...
export async function GET(req: Request) {
  ...
  const result = await handleIncrement({ pageId: pageId ?? undefined, slug: slug ?? undefined });
  if (!result.ok) return NextResponse.json(result, { status: result.status ?? 500 });
  return NextResponse.json(result);
}
