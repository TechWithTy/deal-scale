...
export async function GET(request: NextRequest) {
  try {
    ...
    const perPage = Math.max(1, Math.min(100, Number(search.get("per_page")) || Number(search.get("limit")) || 12));
    ...
    const fetchInit: RequestInit = cacheParam === "off" ? { cache: "no-store" } : { next: { revalidate: 900 } } as any;
    const res = await fetch(postsUrl, fetchInit);
    if (!res.ok) {
      return NextResponse.json({ data: [], message: "Failed to fetch posts for leaderboard" }, { status: res.status });
    }
    ...
    const ranked = [...posts].sort((a, b) => {
      ...
      return toTime(b?.published_at ?? b?.publish_date) - toTime(a?.published_at ?? a?.publish_date);
    });
    ...
    return NextResponse.json({ data: [], message: "Server error building leaderboard" }, { status: 500 });
  }
}
