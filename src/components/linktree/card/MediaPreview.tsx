...
export function MediaPreview({ imageUrl, thumbnailUrl, videoUrl, files }: MediaPreviewProps) {
  const firstImageFromFiles = React.useMemo(
    () => firstMatch(files, (f) => f.kind === "image" || /\.(jpg|jpeg|png|gif|webp|avif|svg|heic|heif)(?:$|\?|#)/i.test(f.url)),
    [files],
  );
  const firstVideoFromFiles = React.useMemo(
    () => firstMatch(files, (f) => f.kind === "video" || /\.(mp4|webm|ogg|mov|m4v)(?:$|\?|#)/i.test(f.url)),
    [files],
  );
  ...
  const proxiedVideo = rawVideo ? `/api/proxy-video?url=${encodeURIComponent(rawVideo)}` : undefined;

  // Prefer image by default
  if (rawImage) {
    const heic = /(\.heic|\.heif)(?:$|\?|#)/i.test(rawImage ?? "");
    const cloud = (typeof process !== "undefined" ? process.env.CLOUDINARY_CLOUD_NAME : undefined) as string | undefined;
    const imgSrc = heic && cloud ? `https://res.cloudinary.com/${cloud}/image/upload/f_jpg,q_auto/${encodeURIComponent(rawImage)}` : rawImage;
    return (
